classdef pAOSOLogitBoost
  ...
  methods
    function obj = train(obj,X,Y, varargin)
      ...
      obj.ptr = pAOSOLogitBoost_mex('train',...
          X,Y,var_cat_mask,...
        T, J, v,...
        node_size);
    end
    ...
    function Y = predict(obj, X, T)
      if (nargin==2) 
        [T,~] = pAOSOLogitBoost_mex('get',obj.ptr);
      end
      Y = pAOSOLogitBoost_mex('predict', obj.ptr, X, T);
    end
    
    function delete(obj)
      pAOSOLogitBoost_mex('delete',obj.ptr);
    end
  end % method
